...
var ANNOTATION_REGEX = new RegExp('^\\s*@(\\D*)$');
...
var FeatureParser = function(language) {
    ...
    function parse_line(line, index) {
        ...
        try {
            ...
            if (match = ANNOTATION_REGEX.test(line)) return specification.handle('Annotation', { key: StringUtils.trim(line) }, line_number);
...
var Annotations = function() {
    ...
    this.stash = function(line) {
        var keys = line.split(' ');
        keys.forEach(function(eachKey) {
          var tag = SIMPLE_ANNOTATION_REGEX.exec(eachKey);
          var tagWithArg = NVP_ANNOTATION_REGEX.exec(eachKey);

          if (tag) {
            annotations[tag[1].toLowerCase()] = true;
          } else if (tagWithArg) {
            annotations[tagWithArg[1].toLowerCase()] = tagWithArg[2];
          }
        });
    };
